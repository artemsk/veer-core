...
trait EcommerceTrait {
	 ...
	 */
	protected function sendEmailOrdersStatus($orderId, $options = [])
	{
		...
		$data_array['status'] = \Illuminate\Support\Arr::get($options, 'history');
		$data_array['link'] = $data->site->url . "/order/" . $orderId;

		$subject = \Lang::get('veeradmin.emails.order.subject', ['oid' => $data_array['orders_id']]);
