<?php

namespace Veer\Models;

class Category extends \Eloquent {
    
    protected $table = "categories";
	
    use \Illuminate\Database\Eloquent\SoftDeletes; 	
	protected $dates = ['deleted_at'];
   
    public function scopeSiteValidation($query, $site_id) {
        return $query->whereHas('categories', function($q) use ($site_id) {
                $q->where('sites_id','=',$site_id); // @todo remember 3
            });
    }    
        
    // Many Categories Have One Site
    
    public function site() {
        return $this->belongsTo(\Veer\Models\Site::class,'sites_id','id');
    }
    
    // Many Categories Have Many (Sub/Parent Categories, Pages, Products & Images)
    
    public function subcategories() {
        return $this->belongsToMany(\Veer\Models\Category::class,'categories_pivot', 'parent_id','child_id');
    }
    
    public function parentcategories() {
        return $this->belongsToMany(\Veer\Models\Category::class,'categories_pivot', 'child_id','parent_id');
    }
    
    public function pages() {
        return $this->morphedByMany(\Veer\Models\Page::class, 'elements', 'categories_connect', 'categories_id', 'elements_id');
    }

    public function products() {
        return $this->morphedByMany(\Veer\Models\Product::class, 'elements', 'categories_connect', 'categories_id', 'elements_id');
    }
    
    public function images() {
        return $this->morphToMany(\Veer\Models\Image::class, 'elements', 'images_connect', 'elements_id', 'images_id')->withPivot('id');
    }
    
    // One Category Has Many Messages
    
    public function communications() {
        return $this->morphMany(\Veer\Models\Communication::class, 'elements');
    }   
               
}